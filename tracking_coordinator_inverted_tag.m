...
videopath = 'C:\Users\bastien.lemaire\Desktop\Beetag_alldays_file\videos' ;
txtfilepath = 'C:\Users\bastien.lemaire\Desktop\Beetag_alldays_file\BEETag_for_chicks\txtfiles_chicks' ;
outputpath = 'C:\Users\bastien.lemaire\Desktop\Beetag_alldays_file\results' ;
...
for i = periodtoanalyze
    ...
    for ii = 1:size(frames,1)
        ...
        for v = 1:bins_session
            ...
            trackingDataoutput1 = trackBEEtagsAcrossFrames(video.name, brThresh, brFilt, new_frames, codelist );
            ...
            for p = new_frames
                ...
                if p==new_frames(length(new_frames)) && isempty(F)==1
                    session_trackk(1).CentroidX(p) = 0;
                    session_trackk(1).CentroidY(p) = 0;
                    session_trackk(1).FrontX(p) = 0;
                    session_trackk(1).FrontY(p) = 0;
                    session_trackk(1).number(p) = 0;
                end
                
                % Uncomment this when the tag has been inverted on the back
                % of the chick
                session_track(1).CentroidX = session_trackk(1).FrontX;
                session_track(1).FrontX = session_trackk(1).CentroidX;
                session_track(1).CentroidY = session_trackk(1).FrontY;
                session_track(1).FrontY = session_trackk(1).CentroidY;
                session_track(1).number = session_trackk(1).number;
